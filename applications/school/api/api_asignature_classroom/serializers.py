...
class AsignatureClassroomSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        asignature_knowledge_area_id = Asignature.objects.get_asignature_by_id(
            attrs['asignature'].id
        )
        ...
        area = KnowledgeArea.objects.get_teachers_ids_by_area(
            asignature_knowledge_area_id.knowledge_area.id
        )
    ...
    def update(self, instance, validated_data):
        ...
        if instance.asignature != validated_data['asignature']:
            raise serializers.ValidationError(
                {
                    'asignature': 'Error, una vez ingresada la Asignatura no se puede cambiar '
                                  'el mismo.'
                }
            )
