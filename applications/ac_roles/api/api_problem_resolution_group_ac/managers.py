
from django.db import models


class ProblemResolutionGroupAcManager(models.Manager):

    def get_problem_resolution_ac_active_queryset(self):
        return self.select_related('team_ac').filter(
            team_ac__active=True,
            team_ac__auth_state='A',
            active=True,
            auth_state='A'
        )

    def get_problem_resolution_ac_active_object_queryset(self, pk=None):
        try:
            return self.select_related('team_ac').filter(
                id=pk,
                team_ac__active=True,
                team_ac__auth_state='A',
                active=True,
                auth_state='A'
            )
        except:
            return None

    def get_problem_resolution_ac_list(self):
        return self.get_problem_resolution_ac_active_queryset().order_by('-created_at')

